...
impl Database {
        ...
        let result = (|| {
            while version < SCHEMA_VERSION {
                match version {
                    ...
                    10 => {
                        log::info!("迁移数据库从 v10 到 v11（添加 Grok 代理支持）");
    ...
    /// v10 -> v11 迁移：添加 Grok 代理支持
    fn migrate_v10_to_v11(conn: &Connection) -> Result<(), AppError> {
        Self::rebuild_proxy_config_for_apps(conn)?;
        log::info!("v10 -> v11 迁移完成：已添加 Grok 代理配置行");
        Ok(())
    }

    fn rebuild_proxy_config_for_apps(conn: &Connection) -> Result<(), AppError> {
        if !Self::table_exists(conn, "proxy_config")? {
            return Ok(());
        }
        if !Self::has_column(conn, "proxy_config", "app_type")? {
            return Self::migrate_proxy_config_to_per_app(conn);
        }
        ...
        let insert_columns = column_defaults
            .iter()
            .map(|(column, _)| *column)
            .collect::<Vec<_>>()
            .join(", ");
        let select_columns = column_defaults
            .iter()
            .map(|(column, default)| {
        ...
        let copy_sql = format!(
            "INSERT OR IGNORE INTO proxy_config_new ({insert_columns})
             SELECT {select_columns}
             FROM proxy_config
             WHERE app_type IN ('claude','codex','gemini','grok')"
        );
        ...
        .map_err(|e| AppError::Database(format!("插入 Grok proxy_config 失败: {e}")))?;
